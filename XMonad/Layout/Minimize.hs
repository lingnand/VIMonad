...
data MinimizedWindows = MinimizedWindows [Window] deriving (Read, Show, Typeable)
instance ExtensionClass MinimizedWindows where
    initialValue = MinimizedWindows []
    extensionType = PersistentExtension

getMinimizedWindows l = do
    res <- handleMessage l (SomeMessage MinimizedSave) `catchX` return Nothing
    case res of
         Just _ -> do
             MinimizedWindows wins <- XS.get
             return wins
         _ -> return []
...
instance LayoutModifier Minimize Window where
    ...
    handleMess ol@(Minimize minimized unfloated) m
