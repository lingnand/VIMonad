...
module XMonad.Util.WorkspaceCompare ( WorkspaceCompare, WorkspaceSort
                                    ...
                                    , getSortByXineramaRuleWithCustomStringSort
...
getXineramaWsCompare = getXineramaWsCompare' compare False
...
getXineramaPhysicalWsCompare = getXineramaWsCompare' compare True

getXineramaWsCompare' :: WorkspaceCompare -> Bool -> X WorkspaceCompare
getXineramaWsCompare' customSort phy = do
    w <- gets windowset
    return $ \ a b -> case (isOnScreen a w, isOnScreen b w) of
        (True, True)   -> cmpPosition phy w a b
        (False, False) -> customSort a b
        (True, False)  -> LT
        (False, True)  -> GT
  where
...
getSortByXineramaRuleWithCustomStringSort :: WorkspaceCompare -> X WorkspaceSort
getSortByXineramaRuleWithCustomStringSort custom = mkWsSort $ getXineramaWsCompare' custom False
